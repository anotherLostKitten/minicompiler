...
public final class AssemblyParser {
  ...
  public static AssemblyProgram readAssemblyProgram(final BufferedReader reader){
	...
	for(var line:reader.lines().collect(Collectors.toList())){
	  ...
	  if(item.equals(textDirective)||item.equals(dataDirective)){
		...
		currentSection=new AssemblyProgram.Section(item.equals(dataDirective)?AssemblyProgram.Section.Type.DATA:AssemblyProgram.Section.Type.TEXT);
	  }else{
		if(currentSection==null)
  ...
  public static AssemblyItem parseAssemblyItem(String line){
	...
	if(line.startsWith("#")){//comments
	...
	}else if(line.startsWith(".")){//directives
	  return new Directive(line.substring(1));
	} else if (line.endsWith(":")){//labels
	...
	}else{//instruction: opcode arg1, arg2, ...
	  ...
	  if(args.size()==1&&args.get(0).isBlank())//parse args
		args=List.of();
	  var op=OpCode.tryParse(opcodeAndArgs[0].trim());//get opcode
	  if(op.isEmpty())
  ...
  private static Register parseRegister(String name){
	...
	if(name.startsWith("$")){
	  var candidate=Register.Arch.allRegisters.stream().filter(r->r.toString().equals(name)).findFirst();
  ...
  private static boolean isLabel(String text){
	return text.length()>0&&Character.isJavaIdentifierStart(text.codePointAt(0))&&text.codePoints().allMatch(Character::isJavaIdentifierPart);
  }
